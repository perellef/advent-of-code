...
while True:
    score, pos, retning = heapq.heappop(lst)
    x,y = pos
    ...
    if (pos, retning) in besøkte:
        continue    
    besøkte[(pos, retning)] = score

    dx,dy = retninger[retning][0]

    heapq.heappush(lst, (score+1, (x+dx,y+dy), retning))
    for ny_retning in retninger[retning][1]:
        heapq.heappush(lst, (score+1000, (x,y), ny_retning))
