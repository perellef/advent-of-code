...
stimatrise = defaultdict(set)
for r,k in høyder[9]:
    stimatrise[(r,k)] = set(((r,k),))

retninger = [(0,1),(0,-1),(1,0),(-1,0)]

for h in range(9,0,-1):
    for r,k in høyder[h-1]:
        stimatrise[(r,k)] = set(e for  dr,dk in retninger for e in stimatrise[(r+dr,k+dk)] if (r+dr,k+dk) in høyder[h])
        
print(sum(len(stimatrise[(r,k)]) for (r,k) in høyder[0]))
# 682